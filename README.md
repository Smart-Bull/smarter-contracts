...
1. Slots can only bind to one signal, and that signal must already exist on the network prior to (or during, if both the signal and the slot are in the same contract) deployment
2. Slots cannot unbind from a signal once binded
...
```
// packages/augur-core/src/contracts/reporting/Universe.sol
